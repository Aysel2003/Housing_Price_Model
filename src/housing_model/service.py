...
from .schema import REQURIED_COLUMNS, SchemaError, validate_dataframe
...
async def lifespan(app: FastAPI):
    ...
    pred.allow_extra_columns = bool(
        serve_cfg["behavior"].get("allow_extra_columns", False)
    )
    ...
    logger.info(
        "Service started. model=%s profile=%s profile_loaded=%s",
        model_path,
        profile_path,
        "yes" if pred.training_profile else "no",
    )
...
def meta(request: Request):
    ...
    return {
        ...
        "required_columns": REQURIED_COLUMNS,
    }
...
def predict(req: PredictRequest, request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail={"error": str(e), "details": e.details}
        )
